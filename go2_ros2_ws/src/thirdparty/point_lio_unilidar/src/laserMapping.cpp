...
inline void dump_lio_state_to_log(FILE *fp) {
  ...
  if (use_imu_as_input) {
    ...
    fprintf(fp, "%lf %lf %lf ", kf_output.x_.gravity(0),
            kf_output.x_.gravity(1), kf_output.x_.gravity(2));
  }
  fprintf(fp, "\r\n");
  fflush(fp);
}
...
void pointBodyLidarToIMU(PointType const *const pi, PointType *const po) {
  ...
  if (extrinsic_est_en) {
    if (!use_imu_as_input) {
      p_body_imu = kf_output.x_.offset_R_L_I.normalized() * p_body_lidar +
                   kf_output.x_.offset_T_L_I;
    } else {
      p_body_imu = kf_input.x_.offset_R_L_I.normalized() * p_body_lidar +
...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg) {
  ...
  for (const auto &pt : ptr->points) {
    ...
    if (dist <= max_range_crop) {
      ptr_cropped->points.push_back(pt);
    }
  }
  ptr_cropped->width = ptr_cropped->points.size();
  ptr_cropped->height = 1;
  ...
  if (!ptr->points.empty()) {
    ...
    Eigen::Vector3d ang_vel(imu_last.angular_velocity.x,
                            imu_last.angular_velocity.y,
                            imu_last.angular_velocity.z);

    for (auto &pt : ptr->points) {
      double rel_time =
          pt.curvature / 1000.0; // curvature holds relative time (in ms)
      simpleUndistortPoint(pt, ang_vel, rel_time);
    }
  }

  if (!ptr->points.empty()) {
    const auto &pt_first = ptr->points.front();
    const auto &pt_last = ptr->points.back();
    double dx = pt_first.x - pt_last.x;
  ...
  if (cut_frame) {
    ...
    for (int i = 0; i < ptr->size(); i++) {
      ...
      if (ptr->points[i].curvature / double(1000) +
              get_time_in_sec(msg->header.stamp) - time_div >
...
//             for (int i = 0; i < ptr->size(); i++)
//             {
//                 ptr->points[i].curvature += (last_timestamp_lidar - time_con)
//                 * 1000; ptr_con->push_back(ptr->points[i]);
//             }
//             frame_ct++;
//         }
//         else
...
void imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr msg_in) {
  ...
  msg->header.stamp = get_ros_time(get_time_in_sec(msg_in->header.stamp) -
...
void publish_odometry(
  ...
  const double publish_interval = 1.0 / 100.0; // 100 Hz
  ...
  {
      now_time = lidar_end_time;
  }

  if (now_time - last_pub_time < publish_interval)
  {
  ...
  trans_odom_to_base.transform.translation.x =
      odomAftMapped.pose.pose.position.x;
  trans_odom_to_base.transform.translation.y =
      odomAftMapped.pose.pose.position.y;
  trans_odom_to_base.transform.translation.z =
      odomAftMapped.pose.pose.position.z;
  trans_odom_to_base.transform.rotation.w =
      odomAftMapped.pose.pose.orientation.w;
  trans_odom_to_base.transform.rotation.x =
      odomAftMapped.pose.pose.orientation.x;
  trans_odom_to_base.transform.rotation.y =
      odomAftMapped.pose.pose.orientation.y;
  trans_odom_to_base.transform.rotation.z =
...
int main(int argc, char **argv) {
  ...
  auto sub_imu =
      node->create_subscription<sensor_msgs::msg::Imu>(imu_topic, 10, imu_cbk);
  ...
  auto pubLaserCloudMap =
      node->create_publisher<sensor_msgs::msg::PointCloud2>("/Laser_map", 1);

  auto pubOdomAftMapped =
      node->create_publisher<nav_msgs::msg::Odometry>("/aft_mapped_to_init", 1);

  auto pubPath = node->create_publisher<nav_msgs::msg::Path>("/path", 1);
  ...
  while (rclcpp::ok()) {
    ...
    if (imu_en) {
      if (!p_imu->gravity_align_) {
        while (Measures.lidar_beg_time >
               get_time_in_sec(imu_next.header.stamp)) {
          imu_last = imu_next;
          imu_next = *(imu_deque.front());
          imu_deque.pop_front();
        }
    ...
    if (!use_imu_as_input) {
      ...
      for (k = 0; k < time_seq.size(); k++) {
        ...
        if (imu_en) {
          bool imu_comes =
              time_current > get_time_in_sec(imu_next.header.stamp);
          while (imu_comes) {
            imu_upda_cov = true;
            ...
            imu_next = *(imu_deque.front());
            imu_deque.pop_front();
            double dt = get_time_in_sec(imu_last.header.stamp) -
            ...
            {
              double dt_cov =
                  get_time_in_sec(imu_last.header.stamp) - time_update_last;
    ...
    if (runtime_pos_log) {
      ...
      printf("[ mapping ]: time: IMU + Map + Input Downsample: %0.6f ave "
             "match: %0.6f ave solve: %0.6f  ave ICP: %0.6f  map incre: %0.6f "
             "ave total: %0.6f icp: %0.6f propogate: %0.6f \n",
    ...
    // if (!feats_down_body->points.empty()) {
    //   auto &pt_first = feats_down_body->points.front();
    //   auto &pt_mid =
    //       feats_down_body->points[feats_down_body->points.size() / 2];
    //   auto &pt_last = feats_down_body->points.back();

    //   double time_first =
    //       Measures.lidar_beg_time + (pt_first.curvature / 1000.0);
    //   double time_mid = Measures.lidar_beg_time + (pt_mid.curvature / 1000.0);
    //   double time_last = Measures.lidar_beg_time + (pt_last.curvature / 1000.0);

    //   double ros_now_sec = node->now().seconds();

    //   std::cout << std::fixed;
    //   std::cout << "[Delay] First point: "
    //             << (ros_now_sec - time_first) * 1000.0 << " ms" << std::endl;
    //   std::cout << "[Delay] Mid point: " << (ros_now_sec - time_mid) * 1000.0
    //             << " ms" << std::endl;
    //   std::cout << "[Delay] Last point: " << (ros_now_sec - time_last) * 1000.0
    //             << " ms" << std::endl;
    // }
    ...
    // std::cout << "[Scan Time] Processing duration: " << scan_duration << " ms"
    //           << std::endl;
