...
void ImuProcess::IMU_init(const MeasureGroup &meas, int &N) {
  ...
  std::cout << "IMU Initializing: " << double(N) / MAX_INI_COUNT * 100 << "%"
            << std::endl;
  V3D cur_acc, cur_gyr;
...
void ImuProcess::Process(const MeasureGroup &meas,
  ...
  // if (imu_need_init_){
  //   std::cout << "imu_en: " << imu_en << ", imu_need_init:" << imu_need_init_
  //   << std::endl;
  // }
  if (imu_en) {
...
void ImuProcess::Set_init(Eigen::Vector3d &tmp_gravity, Eigen::Matrix3d &rot) {
  ...
  double align_norm =
      (hat_grav * tmp_gravity).norm() / tmp_gravity.norm() / gravity_.norm();
...
inline void simpleUndistortPoint(PointType &pt, const Eigen::Vector3d &ang_vel,
                                 double rel_time) {
  // rel_time is in seconds, positive if later in scan
  Eigen::AngleAxisd rot(rel_time * ang_vel.norm(), ang_vel.normalized());
  Eigen::Vector3d p(pt.x, pt.y, pt.z);
  Eigen::Vector3d p_rot = rot.inverse() * p; // undo the rotation
  pt.x = p_rot.x();
  pt.y = p_rot.y();
  pt.z = p_rot.z();
}